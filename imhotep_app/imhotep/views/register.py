...
class RegisterView(QWidget):
    ...
    def __init__(self):
        ...
        #self.back_btn.clicked.connect(lambda: print("Back clicked"))

        # Title
        self.title = QLabel("Imhotep"); self.title.setFont(QFont("Segoe UI", 26, QFont.Bold)); self.title.setAlignment(Qt.AlignCenter)
        self.subtitle = QLabel("Register your account"); self.subtitle.setFont(QFont("Segoe UI", 12)); self.subtitle.setAlignment(Qt.AlignCenter); self.subtitle.setStyleSheet("color: gray;")
        ...
        self.eye_button.setCheckable(True); self.eye_button.setCursor(Qt.PointingHandCursor); self.eye_button.setToolTip("Show/Hide Password")
        ...
        # Strength
        self.password_strength = QLabel(""); self.password_strength.setAlignment(Qt.AlignLeft); self.password_strength.setStyleSheet("color: gray; font-size: 12px; padding-left: 5px; font-family: 'Segoe UI';")
        self.strength_bar = QProgressBar(); self.strength_bar.setFixedHeight(6); self.strength_bar.setTextVisible(False); self.strength_bar.setRange(0, 100); self.strength_bar.setValue(0)
        ...
        self.info.setWordWrap(True); self.info.setAlignment(Qt.AlignCenter); self.info.setStyleSheet("color: gray; font-size: 12px; font-family: 'Segoe UI';")
        self.notification = QLabel(""); self.notification.setAlignment(Qt.AlignCenter); self.notification.setStyleSheet("color: red; font-size: 13px; font-weight: bold; font-family: 'Segoe UI';")
        ...
        btn_layout = QHBoxLayout(); btn_layout.addWidget(self.register_btn); btn_layout.addWidget(self.cancel_btn); btn_layout.setSpacing(20)
    ...
    def update_card_layout(self):
        card_width = int(self.width() * 0.55); card_height = int(self.height() * 0.8)
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event); self.update_card_layout()

    def adjust_eye_position(self):
        self.eye_button.move(self.password.width() - self.eye_button.width() - 10, (self.password.height() - self.eye_button.height()) // 2)

    def toggle_password_visibility(self):
        if self.eye_button.isChecked():
            self.password.setEchoMode(QLineEdit.Normal); self.eye_button.setText("🙈")
        else:
            self.password.setEchoMode(QLineEdit.Password); self.eye_button.setText("👁️")

    # ----- strength -----
    def check_password_strength(self):
        password = self.password.text(); score = 0
        ...
        if score <= 2: strength, color, bar_value = "Weak", "#f44336", 30
        elif 3 <= score <= 4: strength, color, bar_value = "Medium", "#ff9800", 65
        else: strength, color, bar_value = "Strong", "#4CAF50", 100
    ...
    def register_user(self, checked=False):
        ...
        try:
            ...
            if not fullname or not unique_code or not password:
                self.show_notification("Please fill in all fields."); return
            if " " in unique_code:
                self.show_notification("Unique Code cannot contain spaces!"); return
            if not unique_code.isdigit():
                self.show_notification("Unique Code must contain digits only!"); return
            if not (8 <= len(password) <= 16):
                self.show_notification("Password must be 8 to 16 characters long!"); return

            # --- connect via PyMySQL (MariaDB-friendly) ---
